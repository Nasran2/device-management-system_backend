...
class ShopController extends Controller{
    ...
    public function store(Request $r,AuditService $audit){$this->super($r);$d=$this->validateData($r,true);[$shop,$user]=DB::transaction(function()use($d,$r){$shopData=collect($d)->except('temporary_password', 'username')->all();$shopData['reference_code']=($d['reference_code']??null)?:'SHOP-'.strtoupper(Str::random(8));$shopData['created_by']=$r->user()->id;$shop=Shop::create($shopData);$username=$d['username']??Str::slug($d['owner_name'],'.').'.'.strtolower(Str::random(4));$user=User::create(['shop_id'=>$shop->id,'name'=>$d['owner_name'],'username'=>$username,'email'=>$d['email'],'phone'=>$d['mobile'],'business_name'=>$d['name'],'address'=>$d['address'],'password'=>$d['temporary_password'],'role'=>'shop_owner','is_active'=>$d['status']==='active']);return[$shop,$user];});$audit->record('SHOP_CREATED','Shop owner account created',$r->user(),null,[],['shop_id'=>$shop->id]);return redirect()->route('shops.show',$shop)->with('success','Shop and separate Shop Owner login created.');}
    ...
    public function resetPassword(Request $r,Shop $shop,AuditService $audit){$this->super($r);$d=$r->validate(['password'=>['required','current_password'],'temporary_password'=>['required','string','min:5','confirmed'],'reason'=>['required','string','max:1000']]);$owner=$shop->users()->where('role','shop_owner')->firstOrFail();$owner->update(['password'=>$d['temporary_password']]);$audit->record('SHOP_PASSWORD_RESET','Shop Owner password reset: '.$d['reason'],$r->user(),null,[],['shop_id'=>$shop->id,'owner_user_id'=>$owner->id]);return back()->with('success','Shop Owner password reset. Share it securely and require an immediate change.');}
    ...
    private function validateData(Request $r,bool $create){
        ...
        $data=$r->validate([
            ...
            'username'=>['nullable','string','max:50',\Illuminate\Validation\Rule::unique('users')->ignore($owner)],
            ...
            'notes'=>['nullable'],
            'temporary_password'=>[$create?'required':'nullable','string','min:5'],
        ]);
