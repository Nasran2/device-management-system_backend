...
class QrProvisioningService
{
    ...
    public function generate(Device $device, int $userId): array
    {
        return DB::transaction(function () use ($device, $userId) {
            $device->provisioningTokens()->whereIn('status', ['active', 'provisioning_started', 'waiting_for_pin'])->update(['status' => 'revoked', 'revoked_at' => now()]);
            $plain = bin2hex(random_bytes(32));
            $token = $device->provisioningTokens()->create(['token_hash' => hash('sha256', $plain), 'expires_at' => now()->addMinutes(SystemSetting::value('provisioning_qr_expiry_minutes', 30)), 'created_by' => $userId]);
    ...
    public function payload(Device $device, string $plain): array
    {
        ...
        if ($ssid !== '') {
            ...
            $payload['android.app.extra.PROVISIONING_WIFI_HIDDEN'] = (bool) SystemSetting::value('provisioning_wifi_hidden', false);
            if ($security !== 'NONE' && ($encrypted = SystemSetting::value('provisioning_wifi_password_encrypted'))) {
                $payload['android.app.extra.PROVISIONING_WIFI_PASSWORD'] = Crypt::decryptString($encrypted);
            }
        }
...
return $payload;
    ...
    {
        $result = (new Builder(writer: new PngWriter, data: json_encode($payload, JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE | JSON_THROW_ON_ERROR), encoding: new Encoding('UTF-8'), errorCorrectionLevel: ErrorCorrectionLevel::Medium, size: 900, margin: 30))->build();
