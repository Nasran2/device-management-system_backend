...
class SmsService {
    ...
    public function send(string $event,string $recipient,array $vars,?int $shopId=null,?int $customerId=null,?int $deviceId=null,?User $user=null): SmsLog {
        ...
        $log=SmsLog::create(['shop_id'=>$shopId,'customer_id'=>$customerId,'device_id'=>$deviceId,'recipient_number'=>$recipient,'template'=>$event,'message'=>$body,'provider'=>SystemSetting::value('sms_provider','textit.biz'),'sent_status'=>'pending','sent_by'=>$user?->id]);
        $enabled=match($event){'payment_received','balance_completed'=>SystemSetting::value('payment_sms_enabled',true),'installment_due_3_days','installment_due_tomorrow','installment_due_today'=>SystemSetting::value('due_reminder_enabled',true),'payment_overdue'=>SystemSetting::value('overdue_reminder_enabled',true),'platform_settlement'=>SystemSetting::value('settlement_sms_enabled',true),default=>true};
